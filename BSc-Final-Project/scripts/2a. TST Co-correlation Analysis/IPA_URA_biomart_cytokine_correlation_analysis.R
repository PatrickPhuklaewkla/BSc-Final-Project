...
ipa <- read.table("IPA_output_D2-TSTvsSaline.txt", sep = "\t", skip=2, header=T)
...
biomart_go <- getBM(
  ...
  filters = "go",
  values = c("GO:0005125"), # "GO:0004896" for Cytokine Receptor Activity
  mart = ensembl
)
...
ipa_sep_reduced <- ipa_sep_reduced %>%
  group_by(ID) %>%
  mutate("count" = n()) %>%
  ungroup() # add a column with the cluster size
